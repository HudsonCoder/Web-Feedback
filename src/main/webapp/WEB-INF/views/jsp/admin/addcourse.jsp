...
<body class="hold-transition sidebar-mini">
            ...
            <div class="container-fluid">
				...
				<div class="card mb-3">
					<div class="card-header">
						<i class="fas fa-table"></i> Bảng quản lý Khóa học
					</div>

					<div class="card-body">
						<p style="margin: 8px;">${messenger}</p>
						<form id="update" method="post" action="/admin/addcourses" onsubmit="return checkData()">
							<h3>Thêm mới khóa học</h3>
							<div class="form-group">
								<label for="usr">Mã khoá học:</label>
								<input value="${makhoa}" readonly name="makhoa" type="text" class="form-control" id="usr" required pattern="\S+.*"/>
							</div>

							<div class="form-group">
								<label for="tenkhoa">Tên khóa học:</label>
								<input name="tenkhoa" type="text" class="form-control" id="tenkhoa" required pattern="\S+.*"/>
							</div>

							<div class="form-group">
								<label for="noidung">Nội dung học:</label>
								<textarea class="form-control" rows="5" name="noidung" id="noidung" required pattern="\S+.*"/></textarea>
							</div>
    ...
    <script>
        ...
        function checkData() {
            ...
            alert("Bạn đã Thực hiện Thêm khóa học thành công");
            return true;
        }
    </script>
    <script>
