...
<title>Trang chủ - Sửa Form Feedback</title>
...
<%--    <style>--%>
<%--        <%@include file="/WEB-INF/views/css/update.css" %>--%>
<%--    </style>--%>
<style type="text/css">
#myBtn {
	display: none;
...
<body class="hold-transition sidebar-mini">
	<div class="wrapper">
		...
		<div id="content-wrapper">
			<div class="content-wrapper">
				...
				<div class="container-fluid">
					...
					<!-- Breadcrumbs-->
					<ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="#">Quản lý Feedback</a></li>
					...
					<div class="card mb-3">
						...
						<div class="card-body">
							<form class="container" style="margin-top: 10px;" action=""
								method="post" onsubmit="return check()">
								<h5 style="font-weight: bold;">Câu hỏi Online</h5>
								<c:forEach items="${onlines}" var="cauhoi">
									<div class="form-group">
										<label for="usr">Câu hỏi ${cauhoi.thuTu}:</label> <input
											type="text" name="online${cauhoi.thuTu}" class="form-control"
											id="usr-onl" value="${cauhoi.noiDung}" content="123" required
											pattern="\S+.*" />
									</div>
								</c:forEach>

								<br>
								<h5 style="font-weight: bold;">Câu hỏi Offline</h5>
								<c:forEach items="${offlines}" var="cauhoi">
									<div class="form-group">
										<label for="usr-off">Câu hỏi ${cauhoi.thuTu}:</label> <input
											name="offline${cauhoi.thuTu}" type="text"
											class="form-control" id="usr-off" value="${cauhoi.noiDung}"
											required pattern="\S+.*" />
									</div>
								</c:forEach>

								<br>
								<h5 style="font-weight: bold;">Câu hỏi Seminars</h5>
								<c:forEach items="${seminars}" var="cauhoi">
									<div class="form-group">
										<label for="usr-se">Câu hỏi ${cauhoi.thuTu}:</label> <input
											type="text" name="seminar${cauhoi.thuTu}"
											class="form-control" id="usr-se" value="${cauhoi.noiDung}"
											required pattern="\S+.*" />
									</div>
								</c:forEach>
		...
		<script>
			function check() {
				var online = document.getElementById("usr-onl").value;
				;
				var offline = document.getElementById("usr-off").value;
				;
				var sermina = document.getElementById("usr-se").value;
				;
				if (online == "" || offline == "" || sermina == "") {
					alert("Vui lòng không bỏ trống câu hỏi!");
					return false;
				}
				alert("Bạn đã Chỉnh sửa Câu hỏi thành công");
				return true;
			}
		</script>
		<script>
